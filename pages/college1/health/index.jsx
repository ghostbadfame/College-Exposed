...
const userPage = (props) => {
    ...
    return(
      <>
        ...
        <Navbar userName={ (session?.user?.name) ? session.user.name.split(" ")[0] : 'login' } sectionName="health" info={props.data}/>
        <HeroAcademics title={"Your Health, Your Voice: Share Your Experience with a Health Facility Review!"} subtitle={"The best way to pay it forward is by sharing your experience with those who come after you."} img= {'/assets/images/health-removebg.png'} h={700} w={700}  />
...
export const getServerSideProps = async () => {
    ...
    const res= await axios.get('https://colleges-exposed.vercel.app/api/fetchout?db=test&collection=health')

    const data = await res.data
    
   const promises = data.map(async (x) => {

    const res2 = await axios.get(`https://colleges-exposed.vercel.app/api/fetchout?db=health-reviews&collection=${x.title}`);
