...
const userPage = (props) => {
    ...
    return(
      <>
        ...
        {/*  hero section for health  */}
        <Navbar userName={ (session?.user?.name) ? session.user.name.split(" ")[0] : 'login' } sectionName="hostel" info={props.data}/>
        <HeroAcademics title={"Did you love the cleanliness of the rooms? Was the mess food delicious and varied?"} subtitle={"The best way to pay it forward is by sharing your experience with those who come after you."} img= {'/assets/images/hostel1-removebg.png'} h={900} w={900}  />
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 mb-8">
            ...
            {
                props.data.map && props.healthData.map(item => <Post id={item._id} img={item.img} title={item.title} subtitle = {item.subtitle} description= {item.description} rating={item.reviews} ></Post>)
...
export const getServerSideProps = async () => {
    ...
    const res= await axios.get('https://colleges-exposed.vercel.app/api/fetchout?db=test&collection=hostel')

    const data = await res.data
    
   const promises = data.map(async (x) => {

    const res2 = await axios.get(`https://colleges-exposed.vercel.app/api/fetchout?db=hostel-reviews&collection=${x.title}`);
  ...
  const healthData = await Promise.all(promises);
