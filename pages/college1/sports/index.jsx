...
const userPage = (props) => {
    ...
    return(
      <>
        ...
        {/*  hero section for sports  */}
        <Navbar userName={ (session?.user?.name) ? session.user.name.split(" ")[0] : 'login' } sectionName="sports" info={props.data}/>
        <HeroAcademics title={"Share the Thrill of Victory: Write a Review about Your Favorite Sport."} subtitle={"The best way to pay it forward is by sharing your experience with those who come after you."} img= {'/assets/images/sporto-removebg.png'} h={700} w={700} />
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 mb-8">
            ...
            {
                props.data.map && props.sportsData.map(item => <Post id={item._id} img={item.img} title={item.title} subtitle = {item.subtitle} description= {item.description} rating={item.reviews} ></Post>)
...
export const getServerSideProps = async () => {
    ...
    const res= await axios.get('https://colleges-exposed.vercel.app/api/fetchout?db=test&collection=sports')

    const data = await res.data
    
   const promises = data.map(async (x) => {

    const res2 = await axios.get(`https://colleges-exposed.vercel.app/api/fetchout?db=sports-reviews&collection=${x.title}`);
  ...
  const sportsData = await Promise.all(promises);
...
const getStars = (reviews) => {
  ...
  finalTotalRating=(finalTotalReviews/finalTotalCount)
  ...
  return {stars,rating,totalCount,final_rating};
};
...
function Post({id,img,  title, subtitle, rating}){
    ...
    final_rating += getStars(rating).rating;
